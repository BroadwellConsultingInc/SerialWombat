...
void initUARTSw()
{
	...
	switch (Rxbuffer[0])
	{
		case CONFIGURE_CHANNEL_MODE_0:
			{
				/*TODO configure bitstream, DMA High
...
void updateUARTSwTx()
{
	...
	{
		uartSw->txRemainingNips = updateBitStreamOutput(uartSw->txPin, uartSw->txCurrentBit, uartSw->txRemainingNips);
	}	
	if (uartSw->txRemainingNips)
	{
		return;
	...
	while (moreData)
	{
		while (uartSw->txRemainingNips == 0 && uartSw->txRemainingBits > 0 )
		{
		...
		if (QueueReadByte(uartSw->txQueue,&uartSw->txRemainingBitmap) == QUEUE_RESULT_SUCCESS)
		{
			//Start bit
			uartSw->txCurrentBit = 0; // Start bit low
			uartSw->txRemainingNips = uartSw->divisor;
			uartSw->txRemainingBits = 9;  // 8 plus stop
