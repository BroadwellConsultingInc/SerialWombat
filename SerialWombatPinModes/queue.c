...
#endif
	uint8_t addressCheck = queueAddressValid();
	if (addressCheck != QUEUE_RESULT_SUCCESS)
	{
		return(addressCheck);
	}
	queueByte_t* queue = (queueByte_t*) queueAddress;
...
SW_QUEUE_RESULT_t queueAddressValid()
{
	if ((((uint32_t)queueAddress) & 0x01) != 0)
	{
		return QUEUE_RESULT_UNALIGNED_ADDRESS;
	}
		
	if ((queueAddress >=  (void*)UserBuffer && queueAddress < (void*)&UserBuffer[SIZE_OF_USER_BUFFER]))
	{
		return QUEUE_RESULT_SUCCESS;
	}
	else
	if ((queueAddress >=  (void*) PinUpdateRegisters && queueAddress < (void*) &PinUpdateRegisters[NUMBER_OF_TOTAL_PINS]))
	{
		return QUEUE_RESULT_SUCCESS;
	}
        return QUEUE_RESULT_INVALID_ADDRESS;
